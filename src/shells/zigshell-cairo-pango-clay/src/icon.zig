...
fn scaleToSize(src: *c.cairo_surface_t, size: i32) *c.cairo_surface_t {
    return c.scale_to_size_c(src, size) orelse src; // src fallback shouldn't happen but keeps types happy
}
...
fn tryLoadPng(icon_name: [*:0]const u8, target: i32) ?*c.cairo_surface_t {
    ...
    // Try sized directories, ordered by proximity to target size
    var sized_sorted: [sizes.len]i32 = undefined;
    for (&sized_sorted, 0..) |*ps, i| ps.* = sizes[i];
    std.mem.sort(i32, &sized_sorted, target, struct {
        fn lessThan(t: i32, a: i32, b: i32) bool {
...
fn loadSvgAndRender(path: [*:0]const u8, size: i32) ?*c.cairo_surface_t {
    return c.load_svg_and_render_c(path, size);
}
...
fn tryLoadSvg(icon_name: [*:0]const u8, size: i32) ?*c.cairo_surface_t {
    ...
    var sized_sorted: [sizes.len]i32 = undefined;
    for (&sized_sorted, 0..) |*ps, i| ps.* = sizes[i];
    std.mem.sort(i32, &sized_sorted, size, struct {
        fn lessThan(t: i32, a: i32, b: i32) bool {
            return @abs(a - t) < @abs(b - t);
        }
    }.lessThan);
...
pub fn load(app_id: [*:0]const u8, size: i32) *c.cairo_surface_t {
    ...
    var surf = tryLoadPng(icon_name_ptr, size);
    if (surf) |s| {
    ...
    const result = fallback(app_id, size);
    cacheIcon(app_id_slice, result);
    return result;
}

fn cacheIcon(app_id: []const u8, surf: *c.cairo_surface_t) void {
    if (icon_cache_count >= ICON_CACHE_MAX) clearCache();
...
test "icon hueForString" {
    const hue1 = hueForString("foot");
    const hue2 = hueForString("firefox");
    const hue3 = hueForString("foot"); // should match hue1

    try std.testing.expect(hue1 >= 0.0 and hue1 <= 1.0);
    try std.testing.expect(hue2 >= 0.0 and hue2 <= 1.0);
    try std.testing.expectEqual(hue1, hue3);
    try std.testing.expect(hue1 != hue2);
}
