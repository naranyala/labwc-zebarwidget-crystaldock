...
const AliasEntry = struct { from: []const u8, to: []const u8 };

var app_aliases: [16]AliasEntry = .{
    .{ .from = "foot-term", .to = "foot" },
    undefined, undefined, undefined, undefined, undefined, undefined, undefined,
    undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined,
};
var alias_count: usize = 1;

fn resolveAlias(app_id: []const u8) ?[]const u8 {
    for (0..alias_count) |i| {
        if (std.mem.eql(u8, app_aliases[i].from, app_id)) {
            return app_aliases[i].to;
        }
    }
    return null;
}

pub fn setAlias(from: []const u8, to: []const u8) void {
    for (0..alias_count) |i| {
        if (std.mem.eql(u8, app_aliases[i].from, from)) {
            app_aliases[i].to = to;
            return;
        }
    }
    if (alias_count < app_aliases.len) {
        app_aliases[alias_count] = .{ .from = from, .to = to };
        alias_count += 1;
    }
}
...
// Cache for iconAt() to avoid per-frame O(n*m) item rebuild (#5).
// Populated by draw(), consumed by iconAt(). Invalidated when tops change.
const CachedDockItem = struct {
    app_id: []const u8,
    top_idx: ?usize,
    count: u32,
    focused: bool,
};
var cached_items: [100]CachedDockItem = undefined;
var cached_num_items: usize = 0;
var cached_toggles_w: f64 = 0;
var cached_cache_valid: bool = false;
...
pub fn launchPinned(index: usize) void {
    if (index < persistent_count) {
        ...
        if (pid == 0) {
            const child = c.fork();
            if (child == 0) {
                _ = c.execl("/bin/sh", "sh", "-c", cmd.ptr, @as([*c]const u8, null));
                c.exit(1);
            }
            c.exit(0);
        }
        if (pid > 0) _ = waitpid(pid, null, 0);
    }
}
...
pub fn draw(
    ...
    for (0..@intCast(top_count)) |i| {
        ...
        for (0..num_items) |g| {
            if (std.mem.eql(u8, app_id, items[g].app_id) or
                (resolveAlias(app_id) != null and std.mem.eql(u8, resolveAlias(app_id).?, items[g].app_id)))
            {
    ...
    // hover_idx encoding: -1=none, -2=settings, -3=launcher, -4=home,
    // 0..top_count-1=running top_idx, 1000+g=pinned group g.
    var hovered_g: i32 = -1;
    if (hover_idx >= 1000) {
        hovered_g = hover_idx - 1000;
    } else if (hover_idx >= 0 and hover_idx < top_count) {
        for (0..num_items) |g| {
            if (items[g].top_idx) |ti| {
                if (ti == @as(usize, @intCast(hover_idx))) {
                    hovered_g = @intCast(g);
                    break;
                }
            }
    ...
    // Populate cache for iconAt() to avoid O(n*m) rebuild on every mouse move (#5)
    // Only caches the item grouping (app_id, top_idx) — widths/positions are
    // recomputed in iconAt() because they depend on mouse_x.
    for (0..num_items) |g| {
        cached_items[g] = .{
            .app_id = items[g].app_id,
            .top_idx = items[g].top_idx,
            .count = items[g].count,
            .focused = items[g].focused,
        };
    }
    cached_num_items = num_items;
    cached_toggles_w = toggles_w;
    cached_cache_valid = true;
}
...
pub fn iconAt(w: i32, _: i32, tops: []toplevel.ToplevelInfo, top_count: i32, mouse_x: i32) i32 {
    ...
    // Use cached items from last draw() if valid — avoids O(n*m) grouping
    // rebuild on every mouse move (#5). Cache populated by draw().
    // When valid we only need top_idx; app_id/count/focused are unused for
    // hit-testing.
    var local_items: [100]?usize = undefined;
    const items_slice: []const ?usize = if (cached_cache_valid) blk: {
        for (0..cached_num_items) |g| local_items[g] = cached_items[g].top_idx;
        break :blk local_items[0..cached_num_items];
    } else blk: {
        // Cache miss (first call before any draw). Build from scratch.
        // Identical to draw()'s logic but only stores top_idx.
        var buf: [100]?usize = undefined;
        var ni: usize = 0;
        for (0..persistent_count) |_| { buf[ni] = null; ni += 1; }
        for (0..@intCast(top_count)) |i| {
            const app_id = tops[i].app_id[0..std.mem.indexOfScalar(u8, &tops[i].app_id, 0) orelse tops[i].app_id.len];
            var found = false;
            for (0..ni) |g| {
                const pinned_app_id = if (g < persistent_count) std.mem.sliceTo(&persistent_order[g], 0) else "";
                if (std.mem.eql(u8, app_id, pinned_app_id) or
                    (resolveAlias(app_id) != null and std.mem.eql(u8, resolveAlias(app_id).?, pinned_app_id)))
                {
                    if (buf[g] == null) buf[g] = i;
                    found = true;
                    break;
                }
            }
            if (!found) { buf[ni] = i; ni += 1; }
        }
        break :blk buf[0..ni];
    };
    const num_items = items_slice.len;

    var widths = std.mem.zeroes([100]f64);
    var total_w: f64 = 0;
    const slot: f64 = DOCK_ICON_SIZE + PAD;
    const unscaled_total: f64 = if (num_items > 0) @as(f64, @floatFromInt(num_items)) * slot - PAD else 0;
    const toggles_w: f64 = if (cached_cache_valid) cached_toggles_w else PAD * 4.0 + @as(f64, @floatFromInt(DOCK_ICON_SIZE * 3));
    const unscaled_block_w = unscaled_total + toggles_w;
    const unscaled_start_x: f64 = @max(0, (@as(f64, @floatFromInt(w)) - unscaled_block_w) / 2.0);
    ...
    if (mx >= settings_x and mx < settings_x + @as(f64, @floatFromInt(DOCK_ICON_SIZE))) return -2;
    if (mx >= launcher_x and mx < launcher_x + @as(f64, @floatFromInt(DOCK_ICON_SIZE))) return -4;
    if (mx >= home_x and mx < home_x + @as(f64, @floatFromInt(DOCK_ICON_SIZE))) return -5;
    ...
    for (0..num_items) |g| {
        ...
        if (mx >= icon_center - half_w and mx <= icon_center + half_w) {
            if (items_slice[g]) |idx| return @intCast(idx) else return @as(i32, @intCast(g)) + 1000;
        }
        app_x += icon_w + PAD;
    }
    return -1;
...
test "dock initOrder logic" {
    ...
    // initOrder() no longer hardcodes pinned apps (removed in cleanup); pinned
    // apps are loaded from config instead. It must be idempotent and safe to
    // call when nothing is configured.
    initOrder();
    try std.testing.expectEqual(@as(usize, 0), persistent_count);
    try std.testing.expect(order_initialized);

    // Calling again must not change state.
    initOrder();
    try std.testing.expectEqual(@as(usize, 0), persistent_count);
}
