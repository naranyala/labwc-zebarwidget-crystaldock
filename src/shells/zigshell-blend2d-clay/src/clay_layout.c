...
typedef struct { int t; FB bb; FC bg; float rad; FC tc; int fs; FT tx; FC bc; void* img; FB clip; } FCmd;
#define MC 1024
    ...
    for (int i = 0; i < c->length && fn < MC; i++) {
        ...
        switch (r->commandType) {
            ...
            case CLAY_RENDER_COMMAND_TYPE_IMAGE:
                f->img = r->renderData.image.imageData;
                break;
            case CLAY_RENDER_COMMAND_TYPE_SCISSOR_START:
                f->clip = (FB){ r->boundingBox.x, r->boundingBox.y, r->boundingBox.width, r->boundingBox.height };
                break;
            default: break;
        }
    }
...
int clay_layout_status_bar(int width, int height) {
    ...
    CLAY(CLAY_ID("Root"), .wrapped = { .layout = root, .backgroundColor = { 24, 24, 30, 255 } }) {
        ...
        left.childGap = 12;
        left.padding = CLAY_PADDING_ALL(6);
        CLAY(CLAY_ID("Left"), .wrapped = { .layout = left }) {
            ...
            CLAY_TEXT(CLAY_STRING("OCWS Blend2D-Clay"), .wrapped = { .fontSize = 14, .textColor = { 200, 200, 210, 255 } });
        }
...
int clay_layout_center_card(int width, int height) {
    ...
    CLAY(CLAY_ID("Root"), .wrapped = { .layout = root, .backgroundColor = { 18, 18, 24, 255 } }) {
        ...
        CLAY(CLAY_ID("Card"), .wrapped = {
            ...
            CLAY_TEXT(CLAY_STRING("Clay + Blend2D"), .wrapped = { .fontSize = 20, .textColor = { 240, 240, 250, 255 } });
            CLAY_TEXT(CLAY_STRING("Declarative layout with Blend2D software renderer."), .wrapped = { .fontSize = 13, .textColor = { 160, 160, 175, 255 } });
...
int clay_layout_launcher(int width, int height, int scroll) {
    Clay_BeginLayout();
    Clay_LayoutConfig root = (Clay_LayoutConfig){
        .sizing = { CLAY_SIZING_FIXED(width), CLAY_SIZING_FIXED(height) },
        .childAlignment = { CLAY_ALIGN_X_CENTER, CLAY_ALIGN_Y_CENTER },
    };
    CLAY(CLAY_ID("LRoot"), .wrapped = { .layout = root, .backgroundColor = { 0, 0, 0, 64 } }) {
        Clay_LayoutConfig card = (Clay_LayoutConfig){
            .sizing = { CLAY_SIZING_FIT(0, 0), CLAY_SIZING_FIT(0, 0) },
            .layoutDirection = CLAY_TOP_TO_BOTTOM,
            .childGap = 12,
            .padding = CLAY_PADDING_ALL(20),
            .childAlignment = { CLAY_ALIGN_X_LEFT, CLAY_ALIGN_Y_TOP },
        };
        CLAY(CLAY_ID("LCard"), .wrapped = {
            .layout = card,
            .backgroundColor = { 15, 15, 23, 247 },
            .cornerRadius = CLAY_CORNER_RADIUS(14),
            .border = { .color = { 60, 60, 80, 255 } },
        }) {
            CLAY_TEXT(CLAY_STRING("OCWS Homepage"), .wrapped = { .fontSize = 20, .textColor = { 240, 240, 250, 255 } });

            Clay_LayoutConfig grid = (Clay_LayoutConfig){
                .sizing = { CLAY_SIZING_FIT(0, 0), CLAY_SIZING_FIXED(300) },
                .layoutDirection = CLAY_TOP_TO_BOTTOM,
                .childGap = 8,
            };
            CLAY(CLAY_ID("Grid"), .wrapped = {
                .layout = grid,
                .clip = { .vertical = true, .childOffset = { .x = 0, .y = (float)(-scroll) } },
            }) {
                const int COLS = 4;
                const int ROW_H = 68;
                const int ICON = 48;
                const int ENTRIES = 20;
                for (int r = 0; r * COLS < ENTRIES; r++) {
                    Clay_LayoutConfig row = (Clay_LayoutConfig){
                        .sizing = { CLAY_SIZING_GROW(0, 0), CLAY_SIZING_FIXED(ROW_H) },
                        .layoutDirection = CLAY_LEFT_TO_RIGHT,
                        .childGap = 8,
                    };
                    CLAY(CLAY_IDI("Row", r), .wrapped = { .layout = row }) {
                        for (int c = 0; c < COLS; c++) {
                            int idx = r * COLS + c;
                            Clay_LayoutConfig cell = (Clay_LayoutConfig){
                                .sizing = { CLAY_SIZING_FIXED(96), CLAY_SIZING_FIXED(ROW_H) },
                                .layoutDirection = CLAY_TOP_TO_BOTTOM,
                                .childGap = 4,
                                .childAlignment = { CLAY_ALIGN_X_CENTER, CLAY_ALIGN_Y_CENTER },
                            ...
                            if (idx >= ENTRIES) {
                                CLAY(CLAY_IDI("Cell", idx), .wrapped = { .layout = cell }) {}
                            } else {
                                void* ic = blend_renderer_make_icon(120, 140, 220, 255, ICON);
                                CLAY(CLAY_IDI("Cell", idx), .wrapped = { .layout = cell }) {
                                    Clay_LayoutConfig ip = (Clay_LayoutConfig){
                                        .sizing = { CLAY_SIZING_FIXED(ICON), CLAY_SIZING_FIXED(ICON) },
                                    };
                                    CLAY(CLAY_IDI("Icon", idx), .wrapped = {
                                        .layout = ip,
                                        .image = { .imageData = ic },
                                    }) {}
                                    Clay_LayoutConfig np = (Clay_LayoutConfig){
                                        .sizing = { CLAY_SIZING_GROW(0, 0), CLAY_SIZING_FIT(0, 0) },
                                        .childAlignment = { CLAY_ALIGN_X_CENTER, CLAY_ALIGN_Y_CENTER },
                                    };
                                    CLAY(CLAY_IDI("Name", idx), .wrapped = { .layout = np }) {
                                        CLAY_TEXT(CLAY_STRING("App"), .wrapped = { .fontSize = 11, .textColor = { 220, 220, 230, 255 } });
                                    }
                                }
                            }
                        }
...
void* clay_cmd_img(int i) { return fc[i].img; }
float clay_cmd_clip_x(int i) { return fc[i].clip.x; }
float clay_cmd_clip_y(int i) { return fc[i].clip.y; }
float clay_cmd_clip_w(int i) { return fc[i].clip.w; }
float clay_cmd_clip_h(int i) { return fc[i].clip.h; }
