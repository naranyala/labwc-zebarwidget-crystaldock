...
static GtkWidget *visualizer_area = NULL;
...
// --- Visualizer: 64-bar FFT spectrum + enhanced overlay ---

static gboolean on_draw_visualizer(GtkWidget *w, cairo_t *cr, gpointer d) {
    (void)d;
    ...
    // FFT spectrum bars (64 bars from the audio_analysis FFT)
    if (g_features.n_bins > 0) {
        int num_bars = 64;
        ...
        for (int i = 0; i < num_bars; i++) {
            int fft_idx = (i * (int)(g_features.n_bins / 2)) / num_bars;
            double mag = g_features.spectrum[fft_idx] * 0.4;
    ...
    for (int i = 0; i < 4; i++) {
        ...
        cairo_rectangle(cr, i * bw + 4, height - 10 - h, bw - 8, h);
        cairo_fill(cr);

        cairo_set_source_rgb(cr, 0.8, 0.8, 0.8);
        cairo_move_to(cr, i * bw + 4, height - 16 - h);
        PangoLayout *lay = pango_cairo_create_layout(cr);
        gchar lb[64];
        snprintf(lb, sizeof(lb), "%s %.2f", blabels[i], (double)bands[i]);
    ...
    cairo_arc(cr, g_features.centroid * width, height - 8, 6, 0, 2 * G_PI);
    cairo_fill(cr);
...
static GtkWidget* create_visualizer_tab(void) {
    GtkWidget *vbox = gtk_box_new(GTK_ORIENTATION_VERTICAL, 5);
    visualizer_area = gtk_drawing_area_new();
    gtk_widget_set_size_request(visualizer_area, -1, 300);
    g_signal_connect(G_OBJECT(visualizer_area), "draw", G_CALLBACK(on_draw_visualizer), NULL);
    gtk_widget_add_tick_callback(visualizer_area, on_tick, NULL, NULL);
    gtk_box_pack_start(GTK_BOX(vbox), visualizer_area, TRUE, TRUE, 0);
    return vbox;
}
...
static void activate(GtkApplication *app, gpointer user_data) {
    ...
    gtk_header_bar_set_subtitle(GTK_HEADER_BAR(header), "10-Band EQ & Visualizer");
