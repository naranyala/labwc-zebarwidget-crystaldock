...
Keeps pinned apps in sync across DMS, Noctalia, Zigshell-cairo-pango-dock, and ZIGSHELL-CAIRO-PANGO.
...
ZIGSHELL_WIDGET = os.path.join(HOME, ".config/ocws/widgets/core/dock-apps.widget")
ZIGSHELL_CONFIG = os.path.join(HOME, ".config/zigshell-cairo-pango/panel_1.conf")
...
def get_zigshell_panel():
    if not os.path.exists(ZIGSHELL_CONFIG):
        return None
    try:
        with open(ZIGSHELL_CONFIG, "r", encoding="utf-8") as f:
            for line in f:
        ...
        log(f"Error reading Zigshell-cairo-pango-dock: {e}")
    return None


def set_zigshell_panel(apps):
    if not os.path.exists(ZIGSHELL_CONFIG):
        return
    with open(ZIGSHELL_CONFIG, "r", encoding="utf-8") as f:
        content = f.read()
    ...
    with open(ZIGSHELL_CONFIG, "w", encoding="utf-8") as f:
        f.write(new_content)
...
def get_zigshell_panel():
    ...
    try:
        ...
        log(f"Error reading ZIGSHELL-CAIRO-PANGO: {e}")
    return None


def render_zigshell_widget(apps):
...
button {
  ...
  image {
    ...
    if os.path.exists(os.path.dirname(ZIGSHELL_WIDGET)):
        with open(ZIGSHELL_WIDGET, "w", encoding="utf-8") as f:
            f.write("\n".join(lines))
...
def set_zigshell_panel(apps):
    ...
    with open(OCWS_SETTINGS, "w", encoding="utf-8") as f:
        f.write(new_content)
    render_zigshell_widget(apps)


# === State Management ===

GETTERS = [get_dms, get_noctalia, get_zigshell_panel, get_ocws]
SETTERS = [set_dms, set_noctalia, set_zigshell_panel, set_ocws]
DOCK_NAMES = ["DMS", "Noctalia", "Zigshell-cairo-pango-dock", "ZIGSHELL-CAIRO-PANGO"]
...
def run_daemon():
    ...
    try:
        ...
        last_mtimes = {
            ...
            "zigshell-cairo-pango": get_mtime(ZIGSHELL_CONFIG),
            "zigshell-cairo-pango": get_mtime(OCWS_SETTINGS)
        }
        ...
        while True:
            ...
            m_zig = get_mtime(ZIGSHELL_CONFIG)
            if m_zig > last_mtimes["zigshell-cairo-pango"]:
                last_mtimes["zigshell-cairo-pango"] = m_zig
                c_apps = get_zigshell()
                if c_apps is not None and c_apps != last_apps:
                    log(f"Zigshell-cairo-pango-dock changed: {len(c_apps)} apps")
            ...
            # Check ZIGSHELL-CAIRO-PANGO
            m_ocws = get_mtime(OCWS_SETTINGS)
            if m_ocws > last_mtimes["ocws"]:
                last_mtimes["ocws"] = m_ocws
                s_apps = get_zigshell()
                if s_apps is not None and s_apps != last_apps:
                    log(f"ZIGSHELL-CAIRO-PANGO changed: {len(s_apps)} apps")
